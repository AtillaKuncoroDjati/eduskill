...
document.getElementById('otp-switch').addEventListener('change', function () {
    const form = document.getElementById('form-preferensi-akun');
    ...
    fetch(form.action, {
        ...
        body: JSON.stringify({ is_otp: isEnabled ? 1 : 0 })
    }).then(response => {
        if (!response.ok) {
            return response.json().then(err => { throw err; });
        }
        ...
        if (data.success === true) {
            $.alert({
                title: 'Informasi',
                content: data.message || 'Preferensi akun berhasil diperbarui!',
                type: 'green',
                theme: 'material'
            });
        } else {
            $.alert({
                title: 'Peringatan',
                content: data.message || 'Terjadi kesalahan saat memperbarui preferensi akun.',
                type: 'red',
                theme: 'material',
                backgroundDismissAnimation: 'shake',
                onOpenBefore: function () {
...
document.getElementById('email-notif-switch').addEventListener('change', function () {
    const form = document.getElementById('form-email-notif');
    const isEnabled = this.checked;
    ...
    fetch(form.action, {
        ...
        body: JSON.stringify({ is_email_notification_enabled: isEnabled ? 1 : 0 })
    })
        .then(response => response.json())
        .then(data => {
            if (data.success === true) {
                $.alert({
                    title: 'Informasi',
                    content: data.message || 'Preferensi email berhasil diperbarui!',
                    type: 'green',
                    theme: 'material'
                });
            } else {
                ...
                $.alert({
                    title: 'Peringatan',
                    content: data.message || 'Terjadi kesalahan saat memperbarui preferensi email.',
                    type: 'red',
                    theme: 'material',
                    backgroundDismissAnimation: 'shake',
                    onOpenBefore: function () {
...
// Handle WhatsApp notification submission
document.getElementById('whatsapp-notif-switch').addEventListener('change', function () {
    const form = document.getElementById('form-whatsapp-notif');
    const isEnabled = this.checked;
    const switchElement = this;
    ...
    fetch(form.action, {
        ...
        body: JSON.stringify({ is_whatsapp_notification_enabled: isEnabled ? 1 : 0 })
    })
        .then(response => response.json())
        .then(data => {
            if (data.success === true) {
                $.alert({
                    title: 'Informasi',
                    content: data.message || 'Preferensi WhatsApp berhasil diperbarui!',
                    type: 'green',
                    theme: 'material'
                });
            } else {
                ...
                $.alert({
                    title: 'Peringatan',
                    content: data.message || 'Terjadi kesalahan saat memperbarui preferensi WhatsApp.',
                    type: 'red',
                    theme: 'material',
                    backgroundDismissAnimation: 'shake',
                    onOpenBefore: function () {
